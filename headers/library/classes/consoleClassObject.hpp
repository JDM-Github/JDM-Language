...
public:
	...
	std::shared_ptr<HigherObject> manageFunction(int funcType, std::shared_ptr<HigherObject> &obj1,
		...
		if (newfuncType == ConsoleClass::ConsoleEnum::CONSOLE_RUN) {
			if (obj1->objectValue->fromMainSource)  throw std::runtime_error("Runtime Error: The 'run' method is not static.");
			if (obj1->objectValue->members["isRunning"]->booleanValue) throw std::runtime_error("Runtime Error: The Console is already running.");
			if (objects.empty())       throw std::runtime_error("Runtime Error: Expecting atleast 1 argument.");
			if (objects.size() > 5)    throw std::runtime_error("Runtime Error: Expecting only 5 arguments.");
			if (!objects[0]->isString) throw std::runtime_error("Runtime Error: Console title must be a 'jstring'");

			for (int i = 1; i < objects.size(); i++)
				if (!objects[i]->isInteger) throw std::runtime_error("Runtime Error: Console argument " + std::to_string(i+1) +  "must be a 'jint'");
